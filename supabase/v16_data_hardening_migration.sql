...
begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='wellness_assignments' and column_name='assigned_month') then
    execute $$update public.wellness_assignments set assigned_date = coalesce(assigned_date, assigned_month, created_at::date) where assigned_date is null$$;
  else
    execute $$update public.wellness_assignments set assigned_date = coalesce(assigned_date, created_at::date) where assigned_date is null$$;
  end if;
end$$;
